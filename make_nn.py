...
model = coremltools.models.MLModel("./Models/HandsEmpty.mlmodel")
...
weights = np.zeros(num_classes * 1000)
...
spec.neuralNetworkClassifier.stringClassLabels.Clear()
...
assert len(layer.innerProduct.weights.floatValue) == fan_in * num_classes
assert len(layer.innerProduct.bias.floatValue) == num_classes

# Get the old weights
W_old = np.array(layer.innerProduct.weights.floatValue)
B_old = np.array(layer.innerProduct.bias.floatValue)

# Get random weights with the same shape (Xavier initialization)
...
# Save the new model
coremltools.utils.save_spec(spec, "./Models/Hands2num.mlmodel")
print("Done!")
